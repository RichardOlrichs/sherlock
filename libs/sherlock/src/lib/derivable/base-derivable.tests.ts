...
export interface Factories {
    ...
    error<V>(error: unknown, final?: boolean): Derivable<V>;
}
...
export function testDerivable(factory: Factories | (<V>(atom: Atom<V>) => Derivable<V>), ...modes: DerivableMode[]) {
    ...
    describe('#react', () => {
        ...
        beforeEach(() => {
            value$ = factories.value('the value');
        });
        ...
        if (isSettable) {
            ...
            beforeEach(() => {
                settableValue$ = assertSettable(value$);
            });

            it('should react to change', () => {
                let receivedValue: string | undefined;
                let reactions = 0;
                settableValue$.react(value => {
                    receivedValue = value;
                    reactions++;
                });
                expect(receivedValue).toBe('the value');
            ...
            it('should not recompute when no dependency changed', () => {
                ...
                const computation = jest.fn(<T>(v: T) => v);
    ...
    describe('#connected$', () => {
        ...
        it('should not polute any observer administration', () => {
            ...
            const derived$ = base$.derive(v => v) as Derivation<string>;
    ...
    describe('#toPromise', () => {
        ...
        if (isSettable) {
            ...
            if (isAtom) {
                ...
                beforeEach(() => {
                    atomValue$ = assertDerivableAtom(value$);
                });
    ...
    describe('(final state)', () => {
        ...
        isAtom &&
            it('should react to becoming final', () => {
        ...
        isAtom &&
            it('should disconnect when becoming final', () => {
                const a$ = assertDerivableAtom(factories.value('a'));
                const dummy$ = atom(0);
                const d$ = a$.derive(v => v + dummy$.value);
                d$.react(() => 0);
    ...
    describe('(in debug mode)', () => {
        ...
        noErrorAugmentation ||
            it('should have augmented the error somewhere', () => {
